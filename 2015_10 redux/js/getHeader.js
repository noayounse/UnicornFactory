...
if (image_url[1]) {
	...
	$(image).load(function() {
		...
		function parallax() {
			...
			var newHeight = jumboHeight - scrolled;
			leftSide = $('.projectTitle').offset().left;
			$('.projectBackground').css('height', (newHeight) + 'px');
			var homeLinkTop = 10;
			if (newHeight < 2 * homeLinkTop + homeLinkHeight) {
				homeLinkTop = newHeight - homeLinkTop - homeLinkHeight;
			}
			$('.homeLink').css({top: homeLinkTop + 'px', left:leftSide + 'px'});
		}
